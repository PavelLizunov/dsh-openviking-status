...
export function OpenVikingStatusChip({ sessionId }: { sessionId: string }) {
  ...
  const [sessionData, setSessionData] = useState<OpenVikingSessionData | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      // 2. Session info
      const sessionRes = await fetch(`${DEFAULT_OV_ENDPOINT}/api/v1/sessions/${encodeURIComponent(ovSessionId)}`, {
        headers: { "Content-Type": "application/json" }
      }).catch(() => null);
  ...
  const handleCommitNow = async () => {
    ...
    try {
      const res = await fetch(`${DEFAULT_OV_ENDPOINT}/api/v1/sessions/${encodeURIComponent(ovSessionId)}/commit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const progressPercent = Math.min(100, Math.round((pendingTokens / COMMIT_THRESHOLD) * 100));
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex", alignItems: "center" }} ref={popoverRef}>
      ...
      >
        ...
        />
        <span style={{ fontWeight: 600, color: "var(--dsw-text-default, #e2e8f0)" }}>OV</span>
        <span>
      ...
      {isOpen && (
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
            <span style={{ fontWeight: 600, fontSize: "13px" }}>OpenViking Memory</span>
            <span
              style={{
                fontSize: "10px",
                padding: "2px 6px",
                borderRadius: "4px",
                backgroundColor: health?.ok ? "rgba(52, 211, 153, 0.15)" : "rgba(248, 113, 113, 0.15)",
                color: statusColor,
                fontWeight: 600
              }}
            >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "var(--dsw-text-muted, #94a3b8)" }}>Session:</span>
              <span style={{ fontFamily: "monospace" }}>{ovSessionId.slice(0, 16)}...</span>
            </div>
            {sessionData?.peer_id && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "var(--dsw-text-muted, #94a3b8)" }}>Peer:</span>
                <span style={{ maxWidth: "160px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={sessionData.peer_id}>
                  {sessionData.peer_id}
                </span>
              </div>
            )}
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                <span style={{ color: "var(--dsw-text-muted, #94a3b8)" }}>Pending Tokens:</span>
                <span>{pendingTokens.toLocaleString()} / {COMMIT_THRESHOLD.toLocaleString()}</span>
              </div>
              <div
                style={{
                  width: "100%",
              ...
              >
                <div
                  style={{
                    ...
                    height: "100%",
                    backgroundColor: progressPercent > 80 ? "var(--dsw-status-warning, #fbbf24)" : "var(--dsw-status-success, #34d399)",
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: isCommitting ? "var(--dsw-surface-active, #334155)" : "var(--dsw-surface-base, #1e293b)",
              color: pendingTokens === 0 ? "var(--dsw-text-muted, #64748b)" : "var(--dsw-text-default, #f8fafc)",
              cursor: pendingTokens === 0 || !health?.ok ? "not-allowed" : "pointer",
              fontWeight: 500,
