...
export function OpenVikingStatusChip({ sessionId }: { sessionId: string }) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const healthRes = await defaultOpenVikingClient.checkHealth();
      setHealth(healthRes);
      ...
      const session = await defaultOpenVikingClient.fetchSession(sessionId);
  ...
  const handleCommitNow = async () => {
    ...
    try {
      const res = await defaultOpenVikingClient.commitSession(sessionId, {
